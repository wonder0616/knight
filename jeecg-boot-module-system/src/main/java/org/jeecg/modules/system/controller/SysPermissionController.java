...
import org.jeecg.modules.base.entity.GamUserInfo;
...
public class SysPermissionController {
	...
	@Autowired
	private GamUserPermissionService gamUserPermissionService;
	/**
	 * 加载数据节点
	 * 
	 * @return
...
//			}
	 ...
	 * @return
	 *//*
	@RequestMapping(value = "/getGamUserPermission", method = RequestMethod.GET)
	@ApiOperation(value = "查询gam用户拥有的菜单权限", notes = "查询gam用户拥有的菜单权限")
	public Result<?> getGamUserPermission() {
		Result<JSONObject> result = new Result<JSONObject>();
		try {
			//直接获取当前用户
			LoginUser loginUser = UserContext.getCurrentLoginUser();
			if (oConvertUtils.isEmpty(loginUser)) {
				return Result.error("请登录系统！");
			}
			//根据用户查用户角色
			GamUserInfo gamUserInfo = gamService.queryUserInfo2(loginUser.getId());
			String roleIds = StringUtils.join(gamUserInfo.getRoleIds(),",");
			if(roleIds!="") {
				//根据角色查询所拥有的菜单权限
				Map map = gamService.queryUserPermissionByRoleId(roleIds);
				//Map map1 = (Map) map.get("result");
				JSONArray roles = (JSONArray) map.get("roles");
				List<RoleMenu> permCodes = new ArrayList();
				for (int i = 0; i < roles.size(); i++) {
					JSONObject temp = roles.getJSONObject(i);
					JSONArray funcPerms = temp.getJSONArray("funcPerms");
					for (int j = 0; j < funcPerms.size(); j++) {
						JSONObject funcPerm = funcPerms.getJSONObject(j);
						System.out.println(funcPerm.get("permType"));
						if (funcPerm.get("permType").equals("0")) {
							String permCode =funcPerm.get("permCode").toString();
							RoleMenu roleMenu = this.gamUserPermissionService.getOne(new LambdaQueryWrapper<RoleMenu>().eq(RoleMenu::getCode,permCode.substring(permCode.lastIndexOf("/")+1,permCode.length())).last("limit 1"));
							if(!oConvertUtils.isEmpty(roleMenu))
							{
								permCodes.add(roleMenu);
							}

						}
					}
				}
				JSONArray menujsonArray = (JSONArray) JSONArray.toJSON(permCodes);
				JSONObject json = new JSONObject();

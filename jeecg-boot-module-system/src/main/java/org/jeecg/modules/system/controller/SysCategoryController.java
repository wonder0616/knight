...
public class SysCategoryController {
	...
	public Result<IPage<SysCategory>> queryPageList(SysCategory sysCategory,
		...
		if(oConvertUtils.isNotEmpty(sysCategory.getName())){
			queryWrapper.eq("name", sysCategory.getName());
		}
		if(oConvertUtils.isNotEmpty(sysCategory.getName())){
			queryWrapper.eq("name", sysCategory.getName());
		}
			 ...
			 //按照编码长度排序
              ...
              for (int i = 0; i < listSysCategorys.size(); i++) {
				  ...
				  try {
					  ...
					  successLines++;
					  return Result.ok("文件导入成功！数据行数：" + successLines);
				  } catch (Exception e) {
					  errorLines++;
					  ...
					  if (message.contains(CommonConstant.SQL_INDEX_UNIQ_CATEGORY_CODE)) {
						  errorMessage.add("第 " + lineNumber + " 行：分类编码已存在，忽略导入。");
					  } else {

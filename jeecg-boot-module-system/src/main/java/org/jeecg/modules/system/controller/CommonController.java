...
import org.jeecg.modules.system.util.MagicNumberUtil;
...
public class CommonController {
    ...
    public Result<?> upload(HttpServletRequest request, HttpServletResponse response) {
       ...
       if( MagicNumberUtil.isAudio(value)  || MagicNumberUtil.isVideo(value) || MagicNumberUtil.isImg(value) ||MagicNumberUtil.isText(value) ||file.getOriginalFilename().contains(".txt") ) {
           ...
           if (oConvertUtils.isNotEmpty(savePath)) {
               ...
               fileRelation.setStatus(TPMConstant.FileRelationStatus.NORMAL.getId());
               fileRelation.setFileName(fileName);
               fileRelation.setFileUrl(savePath);
               fileRelationService.save(fileRelation);

               String json_str = JSONObject.toJSONString(fileRelation);

               result.setMessage(json_str);
               result.setSuccess(true);
           } else {
...
//	}
    ...
    public Result fileUrl(@RequestParam("fileUrl") String fileUrl) {

         try {
             String s = Base64.decodeStr(fileUrl);
             fileRelationService.remove(new LambdaQueryWrapper<FileRelation>().eq(FileRelation::getFileUrl,s));
             String substring = s.substring(s.lastIndexOf(bucketName)+bucketName.length() );
             MinioUtil.removeObject(bucketName,substring);
         }catch (Exception e){
